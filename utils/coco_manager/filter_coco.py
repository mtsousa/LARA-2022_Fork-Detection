"""
Adapted from https://github.com/immersive-limit/coco-manager
"""
...
class CocoFilter():
    ...
    def _filter_annotations(self, cont):
        ...
        for image_id, segmentation_list in self.segmentations.items():
            for segmentation in segmentation_list:
                ...
                if original_seg_cat in self.new_category_map.keys():
                    ...
                    if {new_segmentation['category_id']} == cont:
                        self.new_segmentations.append(new_segmentation)
                        if new_segmentation['image_id'] not in self.aux:
                            self.aux.append(new_segmentation['image_id'])
    ...
    def _filter_images(self):
        ...
        for image_id in self.new_image_ids:
            if image_id in self.aux and self.images[image_id] not in self.new_images:
    ...
    def main(self, args):
        ...
        for id in self.new_categories:
            res = {id[key] for key in id.keys() & {'id'}}
            self._filter_annotations(res)
            self._filter_images()
        ...
        # Write the images URLs to a file
        path_name = str(self.output_json_path)
        annotations_name = path_name.split('\\')[-1] if '\\' in path_name else path_name.split('/')[-1]
        file_name = 'annotations/' + annotations_name[:-5] + '_url.txt'
        file = open(file_name, 'w')
        for url in self.new_images:
            res = {url[key] for key in url.keys() & {'coco_url'}}
            file_url = str(res)
            file.write(file_url[2:-2] + '\n')
        file.close()

        # Write the images IDs to a file
        file_name = 'annotations/' + annotations_name[:-5] + '_id.txt'
        file = open(file_name, 'w')
        for img_id in self.new_segmentations:
            res = {img_id[key] for key in img_id.keys() & {'image_id'}}
            file_id = str(res)
            file.write(file_id[1:-1] + '\n')
        file.close()
